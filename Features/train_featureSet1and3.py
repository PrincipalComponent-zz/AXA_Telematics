...
def make_training_set(feature_file, driver_id):
   ...
   feature_input = open(os.path.join(FEATURES,feature_file),'rb')
   feature_csv = csv.reader(feature_input)
   ...
   for row in feature_csv:
      if (driver_id ==0):   # if it is the generate training set, import routes number 5 and 110 for each driver
            ...
            if( int(float(row[2])) == 5 or int(float(row[2])) == 110  or int(float(row[2])) == 151 ):              
...
def Main_Regression():
   ...
   for data_block in data_blocks:   # loop through this many blocks of drivers
      ...
      feature_file = os.path.join(blockfeatures,"Tai_and_Janto_features_block_" + str(data_block) + ".csv")
      ...
      for cnt, driver in enumerate(drivers):
         ...
         if (cnt%10 == 0):
            ...
            if (subset1 in subset_range):
               telematics_file = open(os.path.join(blockresults,"Telematics_results_Tai_and_Janto_Run2_block_"+str(data_block)+ "_subset_"+str(subset1)+".csv"),'wb')
